...
class MarksService:
	...
	async def user_exists(self, id_tg):
		"""
	...
	async def get_user_info(self, id_tg) -> dict:
		"""
	...
	async def save_user_info(self, id_tg: int, user_info: dict):
		"""
	...
	def _abbreviation(self, marks_res: str):
		"""
	...
	def _sort_quater(self, subject_data, subject) -> str:
		"""
	...
	def sort_marks(self, data: dict, period_name) -> str:
		...
		for subject, subject_data in data.get("marks_data").items():
			if subject not in ["finals_average_q", "finals_average_y"]:
				if period_name == "Год":
					result += self._sort_year(subject_data, subject)
				else:
			...
			async with self.session.get(
				...
				if response_marks and response_marks.status == 200:
					...
					if marks.get("marks_data"):
						return self.sort_marks(marks, period)
